...
from . import MatchPattern, PatternType, token
...
class Lexer:
    ...
    def __init__(
        ...
        self._re_ws_skip: PatternType = re.compile(r"\S")
        self._regex_parts: Set[str] = set()
        self._token_list: List[token.Token] = []
    ...
    def _generate_token_from_match(self, regex_match: MatchPattern) -> token.Token:
        token_vars = {}
        ...
        created_token = token.Token(token_name, self._text_buffer_pointer, token_vars)
